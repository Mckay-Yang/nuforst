...
To address these limitations, this study proposes NUFROST (Non-Uniform FFT-based Reconstruction of Optical Satellite Time-series), a robust reconstruction framework that combines the Non-Uniform Fast Fourier Transform (NUFFT) with step-preserving sparse reconstruction. Instead of forcing observations onto a regular temporal grid, NUFROST uses NUFFT to estimate the spectrum of irregularly sampled pixels directly, thereby avoiding interpolation-induced spectral distortion~\cite{ref4,ref6}. The reconstruction stage separates stable harmonic structure from sparse persistent changes: a frequency-tiered Ridge model regularizes unreliable high-frequency components, while a group fused-lasso step term preserves cross-band abrupt events without introducing global Fourier ringing~\cite{ref9,ref15}. The main contributions of this work are summarized as follows:
...
\begin{enumerate}
    ...
    \item A step-preserving reconstruction model that combines frequency-tiered Ridge regularization, joint multi-band outlier screening, and a group fused-lasso innovation term to suppress color speckle, reduce high-frequency ringing, and preserve abrupt persistent land-surface changes.
\end{enumerate}
...
\IEEEpubidadjcol
The remainder of this paper is organized as follows. Section~II reviews related work on time-series reconstruction for optical remote sensing. Section~III details the proposed NUFROST framework, covering NUFFT-based spectral estimation, frequency-tiered harmonic fitting, and step-preserving sparse reconstruction. Section~IV presents the experimental setup, evaluation metrics, and results across reconstruction accuracy, sparse-observation resilience, continuous-gap reconstruction, and ablation analysis. Section~V concludes the paper.
