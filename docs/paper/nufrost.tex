...
Persistent cloud cover often leads to extended periods of missing data, posing a particular challenge for time-series reconstruction. We simulated continuous gaps of varying lengths and evaluated each algorithm's ability to recover the missing segment. As shown in Fig.~\ref{fig_sim} (right panels), NUFROST consistently yields lower gap-filling error and a more stable response as the missing interval grows. The contrast becomes especially clear for more challenging gaps, where Zhu2015 and HANTS are more prone to drift away from the underlying seasonal trajectory while NUFROST better preserves the expected temporal structure.
...
To quantify the contribution of individual components of NUFROST, we performed an ablation study that systematically removed or replaced key elements of the pipeline. The comparison in Fig.~\ref{fig_sim} (bottom panels) contrasts the full method with reduced configurations that omit robust fitting or weaken the frequency-aware treatment of irregular observations and high-frequency components.

The results demonstrate that each component plays a distinct role in the final performance. When robust fitting is weakened, the reconstructions become more sensitive to contaminated observations and residual outliers. When the frequency-aware regularization is simplified, the fitted trajectories become less stable and more susceptible to spurious high-frequency variation. Likewise, replacing direct spectral estimation from irregular samples with a more conventional interpolation-first strategy leads to poorer preservation of the temporal signal. Together, these ablation results support the claim that NUFROST benefits from the joint use of irregular-sampling-aware spectrum estimation, robust loss design, and frequency-weighted regularization.
