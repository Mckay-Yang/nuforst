...
To address these limitations, this study proposes NUFROST (Non-Uniform FFT-based Reconstruction of Optical Satellite Time-series), a robust reconstruction framework that combines the Non-Uniform Fast Fourier Transform (NUFFT) with Huber-Ridge regression. Instead of forcing observations onto a regular temporal grid, NUFROST uses NUFFT to estimate the spectrum of irregularly sampled pixels directly, thereby avoiding interpolation-induced spectral distortion~\cite{ref4,ref6}. To suppress outliers caused by residual clouds and shadows, the framework employs a robust regression model with Huber loss, coupled with a frequency-weighted Ridge penalty that progressively regularizes high-frequency noise~\cite{ref8,ref9}. The main contributions of this work are summarized as follows:

\begin{enumerate}
    \item A reconstruction framework based on NUFFT that estimates the spectrum of irregularly sampled pixels directly, avoiding the spectral distortion inherent in interpolation-based preprocessing.
    \item A hybrid dominant-frequency selection strategy that integrates prior phenological knowledge with data-driven spectral peak detection and parabolic refinement to robustly identify key harmonic components.
    \item A Huber-Ridge regression model with a frequency-weighted penalty that suppresses outliers via iterative reweighting and regularizes high-frequency noise, improving numerical stability under severe temporal gaps.
\end{enumerate}

Experiments on 15 area-of-interest (AOI) scenes across six Sentinel-2 spectral bands demonstrate that NUFROST yields more accurate and stable reconstruction than the HANTS~\cite{ref10} and Zhu2015~\cite{ref14} methods under both sparse-observation and continuous-gap scenarios.

The remainder of this paper is organized as follows. Section~II reviews related work on time-series reconstruction for optical remote sensing. Section~III details the proposed NUFROST framework, covering NUFFT-based spectral estimation and Huber-Ridge robust regression. Section~IV presents the experimental setup, evaluation metrics, and results across reconstruction accuracy, sparse-observation resilience, continuous-gap reconstruction, and ablation analysis. Section~V concludes the paper.
...
Non-uniform spectral estimation provides a promising alternative for irregularly sampled time series. Instead of forcing valid observations onto a regular temporal grid, the Non-Uniform Fast Fourier Transform (NUFFT) estimates spectral components directly from non-uniform observation times~\cite{ref4,ref6}. This property is particularly relevant for optical satellite time series, where the sampling pattern is determined by both revisit cycles and quality masking. However, NUFFT alone only provides a spectral representation; it does not by itself suppress residual atmospheric outliers or prevent high-frequency overfitting under severe temporal gaps. Robust statistical estimation and regularization are therefore necessary for converting non-uniform spectral information into stable pixel-wise reflectance trajectories~\cite{ref8,ref9}.

Motivated by these limitations, this study develops NUFROST, a framework that combines NUFFT-based frequency estimation with Huber-Ridge robust regression for optical time-series reconstruction. The framework avoids interpolation-induced spectral distortion by estimating spectra directly from irregular observations, downweights residual outliers via Huber-based iteratively reweighted fitting, and uses a frequency-weighted Ridge penalty to regularize unreliable high-frequency components.
...
With the extracted dominant frequencies, the time series can be modeled as a linear combination of sinusoidal basis functions, along with a constant and a linear trend. Let $\tilde{t}_j = t_j - \bar{t}$ and denote the $i$-th harmonic basis pair as $\mathbf{h}_i(t_j) = [\cos(2\pi\tilde{\nu}_i t_j), \;\sin(2\pi\tilde{\nu}_i t_j)]$. The design vector for observation $j$ is then constructed as:
\begin{equation}
\label{eq:design}
\mathbf{x}_j = \bigl[1,\; \tilde{t}_j,\; \mathbf{h}_1(t_j),\; \dots,\; \mathbf{h}_m(t_j)\bigr] \in \mathbb{R}^{2m+2}
\end{equation}
The observational model can be written in matrix form as $y \approx X\beta$, where $X$ stacks $\mathbf{x}_j$ for all observations and $\beta$ contains the coefficients for the basis functions in the same order.

Before fitting, the observed reflectance values of each pixel are divided by a robust scale factor $s = \text{median}(|y|)$ to bring them into a normalized range. Normalization ensures that the Huber threshold $\delta$ and the Ridge regularization parameter $\lambda$ operate at comparable scales across spectral bands and surface types with widely different absolute reflectances. After regression, the predicted trajectory is multiplied back by $s$ to recover the original reflectance units.
