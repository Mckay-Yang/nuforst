...
The results demonstrate that each component plays a distinct role in the final performance. When robust fitting is weakened, the reconstructions become more sensitive to contaminated observations and residual outliers. When the frequency-aware regularization is simplified, the fitted trajectories become less stable and more susceptible to spurious high-frequency variation. Additional reductions in the frequency-selection and trend-modeling components also weaken the overall reconstruction, indicating that the full pipeline benefits from combining data-driven spectral selection, robust loss design, and frequency-weighted regularization. Together, these ablation results support the claim that NUFROST performs best when these design choices are used in combination.
