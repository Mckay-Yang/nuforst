...
Many existing reconstruction methods rely on interpolation, piecewise fitting, or harmonic modeling, which can effectively describe seasonal trajectories when observations are sufficiently dense and temporally regular. However, conventional Fourier-based harmonic analysis assumes uniform sampling, so irregular observations are commonly regularized through interpolation or smoothing before modeling. Such preprocessing can introduce pseudo-periodic components across long gaps and distort the true spectral characteristics of the signal~\cite{ref4,ref5}. In addition, residual thin clouds, shadows, and atmospheric contamination often appear as high-frequency fluctuations, to which standard least-squares-based fitting remains sensitive, leading to biased reconstructions and unstable temporal trajectories~\cite{ref6,ref7}.

To address these limitations, this study proposes NUFROST (Non-Uniform FFT-based Reconstruction of Optical Satellite Time-series), a robust reconstruction framework that combines the Non-Uniform Fast Fourier Transform (NUFFT) with Huber-Ridge regression. Instead of forcing observations onto a regular temporal grid, NUFROST uses NUFFT to estimate the spectrum of irregularly sampled pixels directly, thereby avoiding interpolation-induced distortion~\cite{ref4,ref6}. Dominant frequencies are then identified through adaptive energy-based selection with parabolic refinement, and the temporal trajectory is reconstructed using a robust regression model with Huber loss for outlier suppression and a frequency-weighted Ridge penalty for controlling high-frequency noise~\cite{ref8,ref9}. Experiments based on Sentinel-2 Harmonized time series demonstrate that NUFROST yields more accurate and stable reconstruction than traditional methods under both sparse-observation and continuous-gap scenarios. Overall, the proposed method provides a practical framework for reconstructing irregular optical remote sensing time series and improving the reliability of downstream temporal analysis in complex environments.
